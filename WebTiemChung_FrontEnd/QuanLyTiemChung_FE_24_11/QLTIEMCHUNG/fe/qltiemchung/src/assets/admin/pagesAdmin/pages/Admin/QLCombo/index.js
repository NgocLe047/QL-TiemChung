...
const GoiTiemChung = () => {
  ...
  const handleSelectVaccine = (event) => {
    ...
    const selectedVaccineObject = listVaccine.find((vaccine) => vaccine.id === selectedVaccineId);
    setSelectedVaccine(selectedVaccineId);
    setSelectedVaccineName(selectedVaccineObject ? selectedVaccineObject.tenVaccine : "");
  };
///  
...
const handleCreateGoiTiemChung1 = async () => {
  try {
    ...
    const moTaForLoaiTC1 = ` ${selectedVaccineName}`;

    const res = await createGoiTiemChung(id, moTaForLoaiTC1, 0, maLoaiTiemChung);
    console.log("Kiểm tra", res);
    if (res) {
      setIdGoiTC(id);
      setMoTa(moTaForLoaiTC1);
      setGiamGia(0);
     ...
     <tbody>
      ...
      <div className="vaccine-container">
        ...
        <div>
          ...
          <h2>Vaccine</h2>
          {listVaccine.map((vc) => (
            <div key={vc.id} className="vaccine-option">
      <label key={vc.id} className="vaccine-label">
        <input
          type="checkbox"
          value={vc.id}
        ...
        <div className="vaccine-content">
                  <span className="vaccine-title"> {vc.tenVaccine}</span>
